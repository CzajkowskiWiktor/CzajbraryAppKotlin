...
import com.example.czajbraryapp.Models.Order
...
class CheckoutActivity : BaseActivity() {
    ...
    // A global variable for the address details.
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //get all product list from firestore
        getProductList()
    ...
    fun successCartItemsList(cartList: ArrayList<Cart_Item>) {
        ...
        for (product in mBooksList) {
            for (cart in cartList) {
                if (product.book_id == cart.product_id) {
        ...
        tv_checkout_sub_total.text = "$mSubTotal zł"
        // Here we have kept Shipping Charge is fixed as $10
        tv_checkout_shipping_charge.text = "10.0 zł"

        if (mSubTotal > 0) {
            ll_checkout_place_order.visibility = View.VISIBLE

            mTotalAmount = mSubTotal + 10.0
            tv_checkout_total_amount.text = "$mTotalAmount zł"
        } else {
